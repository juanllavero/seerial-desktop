...
export class MPVController {
  ...
  public startMPV(videoPath: string): void {
    ...
    if (os.platform() === "win32") {
      // Windows: HWND
      windowId = nativeHandle.readInt32LE(0).toString();
    } else if (os.platform() === "darwin") {
      // macOS: NSView
      windowId = "0x" + nativeHandle.toString("hex");
    } else {
    ...
    const args = [
      ...
      os.platform() === "win32"
        ? "--input-ipc-server=\\\\.\\pipe\\mpv-pipe" // Windows
        : "--input-ipc-server=/tmp/mpv-socket", // macOS/Linux
      "--log-file=mpv.log",
      `https://seerial.sirjohn.es/video/${videoPath}`,
    ];
